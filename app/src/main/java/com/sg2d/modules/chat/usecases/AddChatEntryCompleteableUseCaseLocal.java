...
public class AddChatEntryCompleteableUseCaseLocal extends SingleUseCase<AddChatEntryCompleteableUseCaseLocal.Input, ChatEntry> {

    private static final String TAG = AddChatEntryCompleteableUseCaseLocal.class.getSimpleName();
    private ChatEntryRepo repo;

    @Inject
    AddChatEntryCompleteableUseCaseLocal(ChatEntryRepo repo) {
        this.repo = repo;
    }
    ...
    public void execute(Input input, SingleObserver<ChatEntry> subscriber) {
        Single.just(input)
                .flatMap(input1 -> repo.addChatEntryLocal(input1.chatEntry))
